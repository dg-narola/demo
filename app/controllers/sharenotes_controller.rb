...
class SharenotesController < ApplicationController
  ...
  def index
    # @sharenotes = Sharenote.all.joins(:note).where(
    # email: current_user.email).where(notes: {status: false})
    @sharenotes = Sharenote.all.includes(:note, :user).where(
